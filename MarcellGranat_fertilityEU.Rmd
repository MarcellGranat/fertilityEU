...
2. kutatási kérdés
bevett változók miként befolyásolják a TTA-t

3. módszertan megnevezése
adattranszformáció (indexek összeállítása, PCA), panel, lasso

4. saját eredmény röviden

2 végső output összefoglalása
\end{abstract}
...
# Fertility: source: Eurostat database ==================================================
...
```{r fig.cap="Total fertility rates through Europe in 2017"}
### Map of TFR ##########################################################################

f_data %>% 
  filter(var == "TOTFERRT" & str_length(geo) == 4 & time == 2017) %>% 
  plot_NUTS2(limits = c(1, 3)) 
```
...
merge(GDL_subnat, health_index) %>% 
  {
    c(
      ...
      scales::percent(cor(x = .$health, y = .$health_index, method = "spearman")^2,
                      accuracy = .01),
  ...
  kable(
    ...
    Human Development Indices provided by GDL and the estimation based on regional life
    expectency"
  )
...
edu_wide <- get_eurostat("edat_lfse_04", time_format = "num") %>% 
  filter(sex == "T" & !str_detect(isced11, "GEN") & 
           !str_detect(isced11, "VOC") & isced11 != "ED3-8" & !str_detect(geo, "TR")
  ) %>% 
  mutate(
...
```{r fig.cap = "Pairwise correlation among TFR and calculation human development indices"}
...
```{r fig.cap = "Correlation between TFR and family benefits"}
dat %>% 
...
```{r fig.cap = "Regression tree explaining the TFR using only the calculated HD indices (cp = 0.01)"}
# Regression trees ======================================================================
...
```{r fig.cap = "Regression tree explaining the TFR using all the mentioned explanatory variables (cp = 0.01)"}
m_part2 <- dat %>% 
  filter(str_length(geo) == 4) %>% 
...
m_panels %>% 
  ...
  labs(x = "Estimated coefficient", y = "Term", color = "Corresponding p-value \u2264 5%") +
...
m_panels %>% 
  lapply(function(output) {
    c(output$tests, nrow(augment(output$OLS)))
  }) %>% 
  reduce(rbind) %>% 
  t() %>% 
  data.frame() %>% 
...
names(dat_plm) %>% 
  ...
  lapply(function(variable){
    ...
    tibble(
      ...
      'P-value' = scales::percent(t$p.value, accuracy = .01)
    )
  }
  ) %>% reduce(rbind) %>% 
  knitr::kable(caption = 'T-tests', digits = 4, 
               align = c('l', 'c', 'c', 'c', 'c', 'c', 'c'))

```

```{r fig.cap = "Number of used observations by countries when the model contains youth unemployment"}
dat_plm %>% 
  na.omit() %>% 
  group_by(geo) %>% 
  summarise(
...
tidy(LASSO) %>%
  ...
  geom_vline(aes(xintercept = log(LASSO$lambda.1se), color = "lambda.1se"), 
             # TODO name the line
             linetype = 2) +
  geom_vline(aes(xintercept = log(LASSO$lambda.min), color = "lambda.min"), 
             linetype = 2) +
  scale_y_continuous(
...
```{r fig.height=6, fig.cap="Estimated coefficient of the fixed panel model controlling for youth unemployment indicators"}
lasso_coefs %>% 
  rename(lasso = coef) %>% 
...
```{r fig.cap = "Number of used observations by countries when the model does not contain youth unemployment"}
dat_plm %>% 
  na.omit() %>% 
  group_by(geo) %>% 
  summarise(
